...
public class RecRecruitPostServiceImpl implements RecRecruitPostService {
    ...
    private int calculatePriorityForMatchingAttributes(RecruitPost post, Set<UserLanguage> userLanguageList, Set<UserInterestField> userInterestFieldList, Set<UserPersonality> userPersonalityList) {
        ...
        if (post.getRecruitFields().stream().anyMatch(field -> userInterestFieldList.contains(field.getField()))) {
            priority++;
        }
        if (post.getRecruitPersonalities().stream().anyMatch(pers -> userPersonalityList.contains(pers.getPersonality()))) {
            priority++;
        }
        return priority;
    }
    ...
    private List<RecRecruitPostDTO> createRecruitPostDTOList(List<RecruitPost> topPosts) {
        ...
        for (RecruitPost topPost : topPosts) {
            ...
            List<RecruitLanguage> recruitLanguages = topPost.getRecruitLanguages();
            Timestamp createdDate = topPost.getCreatedDate();
